 ...
 * 
 * @类名称: CrashHandler
 * @描述: UncaughtException处理类,当程序发生Uncaught异常的时候,由该类来接管程序,并记录发送错误报告.
 * @开发者: andy.xu
 * @时间: 2014-8-19 上午11:10:33
 * 
 */
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private void sendAppCrashReport(final Context context, final String crashReport, final File file) {
		...
		builder.setMessage("抱歉，程序出现异常，是退提交错误？");
		builder.setPositiveButton("提交", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				try {
					...
					intent.putExtra(Intent.EXTRA_SUBJECT, "推聊Android客户端 - 错误报告");
					if (file != null) {
						intent.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(file));
						intent.putExtra(Intent.EXTRA_TEXT, "请将此错误报告发送给我，以便我尽快修复此问题，谢谢合作！\n");
					} else {
						intent.putExtra(Intent.EXTRA_TEXT, "请将此错误报告发送给我，以便我尽快修复此问题，谢谢合作！\n" + crashReport);
					}
					...
					ToastUtil.onShowToast(context, "There are no email clients installed.");
				} finally {
					dialog.dismiss();
					// 退出
